...
Vcut=0;  %cuticular deposition velocity cm/s
 ...
 options = [3:3];  
  ...
  experiment = 'name' %for user to change, does not affect model

%% what variables are changing? 
dt = 2; % timestep in seconds
days = 1; %days to run over. (1 day takes ~ 10 min to run at dt = 2)
hours = 24*days;
minutes = hours*60;
...
depscheme = 'Wesley';%assmax'(assume maximum gmax reach, only depends on SR),'Wesley', 'Emberson', or 'data BEARPEX'
...
%g_min = 0.004; %minimum stomatal opening fraction
...
advection = 'constant'; %'constant' advection, 'Gaussian' or 'sinusoidal'
